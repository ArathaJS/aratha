...
class PutField extends SymbolicValue {
    ...
    getTopBase() {
        let node = this;
        while (node instanceof SymbolicValue && "base" in node) {
            node = node.base;
        }
        return node;
    }
    ...
    toObjectFormula() {
        ...
        if (this.base instanceof SymbolicValue) {
            ...
            switch (this.base.exprType()) {
                case "Val":
                    propsFormula = ["GetProperties", ["id", baseFormula]];
                    break;
                case "Properties":
                    propsFormula = baseFormula;
                    break;
                default:
                    throw new Error("unknown expression type " + this.base.exprType());
            }
        } else {
            propsFormula = valueToPropsFormula(this.base);
        }
        return ["PutField", propsFormula, offsetFormula, valFormula];
    }
    ...
    toFormula() {
        if (this.base instanceof SymbolicValue) {
            ...
            switch (this.base.exprType()) {
                ...
                case "Properties": {
                    if (this.base.getTopBase() instanceof Variable) {
                        // In case our base is a temporary, we need to add a
                        // dynamic constraint: if the base is an object, return
                        // it with modifications applied. Otherwise, return the
                        // original base.
                        const baseFormula = this.base.toFormula();
