...
class StringSplit extends SymbolicValue {
    constructor(base, sep, len = new Constant(undefined)) {
        super();
        this.base = base;
        this.sep = sep;
        this.len = len;
...
class RegExpExec extends SymbolicValue {
    constructor(regex, str) {
        ...
        const genName = () => {
            const name = getTempName("regex.exec");
            this._temps.push(new Temporary(name, "String"));
            return name;
        };
        const genCapture = () => {
            const name = getTempName("regex.capture");
...
class StringMatch extends SymbolicValue {
    constructor(str, regexp) {
        super();
        this.str = str;
        this.regexp = regexp;
    }
    ...
    eval(model) {
        return String.prototype.match.call(this.str.eval(model), this.regexp.eval(model));
    }

    toFormula() {
        const sf = this.str.toStringFormula();
        const rf = this.regexp.toRegexFormula();
        return ["ite",
            ["str.in.re", sf, rf],
            ["store", "EmptyObject", '"0"', sf],
            "null"
        ];
    }
