...
class Variable extends SymbolicValue {
    ...
    getConstraints() {
        ...
        return [new Type(this.type).constraintFor(this.toFormula())];
    }
...
function primitiveToFormula(v) {
    switch (typeof v) {
        ...
        case "number":
            // NOTE: this is only done because we don't support floating point on the SMT-LIB side.
            v = Math.round(v);
            if (v >= 0) {
    ...
    toBooleanFormula() {
        ...
        if (this.left.getType() === Type.NUMBER && this.right.getType() === Type.NUMBER) {
            switch(op) {
                ...
                case ">":
                case "<":
                    return ["<", this.left.toIntegerFormula(), this.right.toIntegerFormula()];
            }
        }
...
class ToString extends SymbolicValue {
    constructor(base) {
        super();
        this.base = base;
    }
    ...
    eval(model) {
        return String(this.base.eval(model));
    }
...
class StringSearch extends StringIndexOf {
    constructor(base, regexp) {
        super(base, new RegExpInstance(regexp, namer++));
    }
...
class ObjectConstruct extends SymbolicValue {
    constructor(base, value) {
        super();
        this.baseId = getObjectId(base);
        this.value = value;
    }
    ...
    toObjectFormula() {
        return this.value.toObjectFormula();
    }

    toFormula() {
        return ["Obj", String(this.baseId)];
    }
}
exports.ObjectConstruct = ObjectConstruct;
exports.ObjectCast = ObjectConstruct;

class ArrayConstruct extends SymbolicValue {
    constructor(result, len) {
        super();
        this.len = len;
        this.baseId = getObjectId(result);
    }
    ...
    eval(model) {
        return new Array(this.len.eval(model));
    }

    toObjectFormula() {
        return ["js.constructArray", this.len.toFormula()];
    }
...
exports.ArrayConstruct = ArrayConstruct;

class StringObjectConstruct extends SymbolicValue {
    ...
    toObjectFormula() {
        return ["js.ToObject", ["Val", this.value.toStringFormula()]];
    }
...
exports.StringObjectConstruct = StringObjectConstruct;

class StringCast extends SymbolicValue {
    constructor(value) {
        super();
        this.value = value;
    }
    ...
    eval(model) {
        return String(this.len.eval(model));
    }
    ...
    toFormula() {
        return ["Val", this.toStringFormula()];
    }
}
