...
exports.CPSolver = class CPSolver {
    ...
    runSolver(opt) {
        if (opt.twoSteps) {
            ...
            if (sigint || !this.checkRes(res, opt.mzn2fznTimeout, opt.abort))
                return undefined;
            res = child_process.spawnSync(this.fzn(),
                opt.fznArgs.concat([opt.fznPath]),
                {timeout: opt.fznTimeout * 1000, detached: true}
            );
            process.on('SIGINT', function() {
                console.log("Caught interrupt signal");
            });
            if (sigint || !this.checkRes(res, opt.fznTimeout, opt.abort))
                return undefined;
        }
        else {
