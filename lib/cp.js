...
//        console.log(vars, this.models)
        ...
        let locs = 0, eprops = 0, iprops = 0, min_loc = 1, model = "";
        ...
        for (let i in data) {
            ...
            if (j !== -1) {
                ...
                if (line[j - 1] === "S") {
                    ...
                    // FIXME: Workaround for not-fixed integer variables.
                    if (line.indexOf("..") != -1 
                    && (id === "LOCS" || id === "REFS")) {
                      let l = line.slice(line.indexOf('['), line.length - 2)
                      l = eval('[' + l.replace(/\.\./g, ".").replace(/\[/g, "").
                          replace(/]/g, "") + ']')
                      if (l.length > 0) {
                          for (let i in l)
                              l[i] = Math.floor(l[i])
...
//        console.log("Pop " + n, this.models);
        ...
        for (const [v, i] of Object.entries(this.builtins))
            if (i >= m)
                delete this.builtins[v];
