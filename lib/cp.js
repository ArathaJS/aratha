    ...
    addConstraint(cons) {
        if (cons instanceof TypeConstraint) {
            if (cons.subject.name) {
                if (cons.value)
                    this.mzn_body.push("constraint TYP_" + cons.subject.name + 
                    " in " + cons.type.toJSType() + ";\n");
                else
                    this.mzn_body.push("constraint not (TYP_" + 
                    cons.subject.name + " in " + cons.type.toJSType() + ");\n");
            }
            else 
               throw new Error("Unknown " + cons);
        }
        else
...
//        console.log("Declared " + name)
    ...
    pop(n) {
        console.log("Pop " + n, this.mzn_body);
        const m = this.mzn_body.length;
        for (var i = 0; i < n; ++i) {
            delete this.mzn_body[m - i - 1];
            // FIXME: Here, decrease also parameters.
        }
    }
