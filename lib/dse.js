...
class ConstraintCollector {
    ...
    _activateConstraint(constraint) {
        ...
        else if (this.incremental || this._unsyncedIndex === 0) {
            this.solver.push(1);
            this._declareVariables(constraint);
    ...
    pop(n=1) {
        ...
        if (!this.solver.isCPSolver() && idx < this._unsyncedIndex) {
            if (this.incremental) {
                ...
                if (!this._declaredVariables.has(expr.name)) {
                    ...
                    if (this.solver.isCPSolver()) {
                        if (expr.type !== Type.TOP)
                            this.solver.declareVar(expr.name, Type(expr.type));
                        else
                            this.solver.declareVar(expr.name);
                    }
                    else {
                        this.solver.declareConst(expr.name, "Val");
                        if (expr.type !== Type.TOP) {
                            this._assert(new Type(expr.type).constraintFor(expr.toFormula()));
                        }
                    }
                }
    ...
    async getModel() {
        if (this.solver.isCPSolver())
            return parseSolution(this.solver.getSolution())
    ...
    async solvePrefix(constraints, prefixLength) {
        if (prefixLength <= 0)
            return { status: "sat", model: {} };
        this._fixSolverStack(constraints, prefixLength);
        var status;
        if (this.solver.isCPSolver()) {
            this.solver.addConstraints(this._constraintStack);
            this._sync();
            status = this.solver.solveSat();
        }
        else
...
class DSE {
    ...
    async execute() {
        ...
        const path = this._program(input.model);
...
console.log("Solver: " + this._solver.constructor.name)
process.stdout.write("Variables: "); console.log(this._collector._declaredVariables)
process.stdout.write("Constraints: "); console.log(this._collector._constraintStack)
        const item = this._workQueue[0];
        let result = await this._collector.solvePrefix(item.constraints, item.step);
        ...
        // console.log(item.constraints.slice(0, item.step).map((x) => sexpr.stringify(x.toFormula())));
        ...
        result = await this._collector.solvePrefix(item.constraints, item.step);
