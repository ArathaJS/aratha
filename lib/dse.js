...
const CP_OPTS = {
    ...
    'mzn2fznTimeout': '3',
    'fznTimeout': '3',
    'mznArgs': ['--no-output-ozn', '-o', 'model.fzn', '--no-output-ozn'],
    'mzn2fznArgs': ['--no-output-ozn', '-o', 'model.fzn', '--no-output-ozn'],
    'fznArgs': ['-max-length', '1000'],  //'-reverse-regex', 'false',
    'twoSteps': true,
    'solve': 'length', // 'typs', 'typ_len', 'str_def',
    'debug': true,
    'abort': false, //false
    'verifyModel': false
}
        ...
        this._collect(expr, (expr) => {
            if (expr instanceof Variable) {
                ...
                if (!this._declaredVariables.has(expr.name)) {
                    top.add(expr.name);
                    if (this.solver.isCPSolver())
                        console.log("FIXME: Ignoring tmp variable")
                    else {
                        this._declaredVariables.add(expr.name);
    ...
    verifyModel(model) {
        for (let i = 0; i < this._constraintStack.length; ++i) {
            ...
            if (!result) {
                ...
                c.value = tmp;
                if (this.solver.isCPSolver() && CP_OPTS.verifyModel)
...
//            console.log("pushing ", constraints[i]);
    ...
    async solvePrefix(constraints, prefixLength, vars) {
        ...
        if (this.solver.isCPSolver()) {
            this._sync();
            status = await this.solver.solveSat(vars, CP_OPTS);
        }
        else
...
//        console.log(item.constraints.slice(0, item.step).map((x) => sexpr.stringify(x.toFormula())));
        if (this._visitedPaths.hasPrefix(item.constraints.slice(0, item.step))) {
            ...
            console.error(item.constraints.slice(0, item.step));
            console.error(this._visitedPaths);
            throw new Error("_generateInput() called on already visited path condition");
        }
//        console.log("Declared variables:", this._collector._declaredVariables)
