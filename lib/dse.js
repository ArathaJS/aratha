...
class ConstraintCollector {
    ...
    _collectObjectConstants(expr) {
        ...
        this._collect(expr, (expr) => {
            if (expr instanceof Constant && _.isObject(expr.value)) {
                ...
                this._assert(["=", ["GetProperties", id], formula]);
    ...
    _getLevelAssertions(level=this._top()) {
        ...
        for (const [id, formula] of level.objectConstants) {
            assertions.push(["=", ["GetProperties", id], formula]);
        }
...
class DSE {
    constructor(solver, program) {
        ...
        this._solver = solver;
        this._collector = new ConstraintCollector(solver, incremental);
        this._program = program;
        this._inputs = [{}];
        this._visitedPaths = new ExecutionPathSet();
        this._unsatCores = new UnsatCoreSet(8);
        this._candidateCores = new UnsatCoreSet();
        this._workQueue = [];
        this.useUnsatCores = false;
    }
