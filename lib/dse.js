...
class TypeConstraint {
    ...
    toFormula() {
        if (this.subject instanceof SymbolicValue) {
            ...
            if (this.type.has(TYPE_MAP[typeof this.subject]) === this.value) {
                return "true";
            } else {
                return "false";
            }
...
function parseModel(model) {
    ...
    function parseObjectModel(expr) {
        if (expr[0] === "_" && expr[1] === "as-array") {
            ...
            const base = parseObjectModel(expr[1]);
            base[expr[2]] = expr[3];
            return base;
                ...
                if (status === "sat") {
                    ...
                    _.forEach(prefix.constraints, (c2) => {
                        if (!c2.isTrueIn(parsed)) {
                            throw new Error("model failed to validate");
                        }
                    });
