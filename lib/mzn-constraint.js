...
function addMiniZincConstraint(cons, model) {console.log(model.SMT_CONS)
    let smt_cons =  '' + cons
    if (smt_cons === "true")
        return;
    if (smt_cons === "false") {
        model.body += "constraint false;\n";
        return;
    }
        ...
        if (expr.base instanceof Constant) {
            ...
            || (typeof(v) == "string" && isIndex(v)))) {
                ...
                model.body += "constraint set_builtin_prop(" + 
                    toTypeSvalAddr(expr.base) + ", " + mznLiteral(v) + ", " + 
                    model.N_IMPL_PROPS + ");\n";
            }
            // Base is unknown.
            model.N_IMPL_PROPS++;
            ...
            model.body +=
                ...
                "P_NAMES[" + i + "] = " + mznLiteral(v) + " else del_prop_at(" 
                + i + ") endif;\n";
        }
        else {
...
function binaryRelToMZN(op, left, right, tx, sx, ty, sy) {
    ...
    switch (op) {
        ...
        default:
            // FIXME: Not sure! Modelled as distinct.
            return "not(strict_eq(" + tx + ", " + sx + ", " + toAddr(left)  + ", "
                                + ty + ", " + sy + ", " + toAddr(right) + "))";
    }
}
