const {
    ...
    StringCharAt, StringIndexOf, StringConcat, StringSlice, RegExpTest
} = require("./symbolic");
...
function toMZNConstraint(cons, solver) {
    ...
    if (cons instanceof TypeConstraint) {
        if (cons.value)
            return "constraint " + toTyp(cons.subject) + " in " + 
                cons.type.toJSType();
        else
            return "constraint not (" + toTyp(cons.subject) + " in " + 
                cons.type.toJSType() + ")";
    }
    if (!cons.value)
        mzn_cons += "not (";
...
function mznRegExp(re) {
  ...
  switch (re[0]) {
      case "str.to.re":
          return "(" + re[1].substr(1, re[1].length - 2) + ")"; //FIXME: Escape special chars!
      case "re.*":
    ...
    if (expr instanceof Constant) {
        ...
        if (suff === "TYP")
            ...
            if (i === "\"0\"")
                ...
                const k = isNaN(j) ? "str2nat(" + i + ")" : Number(j);
        ...
        if (suff === "TYP")
            ...
            const j0 = Number(i0.slice(1, i0.length - 1));
            const k0 = isNaN(j0) ? "str2nat(" + i0 + ")" : Number(j0);
            const i1 = toNum(expr.start);
            const j1 = Number(i1.slice(1, i1.length - 1));
            const k1 = isNaN(j1) ? "str2nat(" + i1 + ")" : Number(j1);
            return "str_sub(" + toRep(expr.base) + ", " +k0+ " + 1, " +k1+ ")";
        }
        else
            return "0"
    }
