const {
    ...
    NewArray, StringIndexOf, RegExpTest
} = require("./symbolic");
...
function fromTRL(expr, suff) {
    ...
    else if (expr instanceof RegExpTest) {console.log(expr)
        if (suff === "TYP")
            return "Bool";
        else if (suff === "REP")
            return "str_reg(" + toRep(expr.str) + ", " + toRep(expr.base) + ")";
        else
            return "0";
    }
    else {
