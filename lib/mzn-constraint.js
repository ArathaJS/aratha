...
function toMZNConstraint(cons, model, mid, builtin_vars, n_impl, n_writes) {
    let smt_cons = "";
    try {
...
//    console.log(smt_cons)
    ...
    if (cons instanceof TypeConstraint) {
        preProcess(cons.subject, model, mid, builtin_vars, n_impl, n_writes);
    ...
    preProcess(expr, model, mid, builtin_vars, n_impl, n_writes);
    N_WRITES = model.N_WRITES;
    FIRST_GET = model.FIRST_GET;
...
function preProcess(expr, model, mid, vars, ip, nw) {
    if (expr instanceof NewArray) {
        preProcess(expr.value, model, mid, vars, ip, nw)
        const v = expr.value;
        if (!(v in vars)) {
          model.N_LOCS++;
          model.N_IMPL_PROPS++;
        ...
        if (expr.base instanceof Constant) {
            preProcess(expr.offset, model, mid, vars, ip, nw);
            ...
            preProcess(expr.base, model, mid, vars, ip, nw);
            preProcess(expr.offset, model, mid, vars, ip, nw);
        }
        ...
        if  (expr.expr instanceof PutField)
            preProcess(expr.expr.base, model, mid, vars, ip, nw);
        else
            preProcess(expr.expr, model, mid, vars, ip, nw);
    }
    else if (expr instanceof Binary) {
        if  (expr.left instanceof PutField && expr.left.base instanceof Variable)    
            preProcess(expr.left.base, model, mid, vars, ip, nw);
        else
            preProcess(expr.left, model, mid, vars, ip, nw);
        if  (expr.right instanceof PutField && expr.right.base instanceof Variable)    
            preProcess(expr.right.base, model, mid, vars, ip, nw);
        else
            preProcess(expr.right, model, mid, vars, ip, nw);
    }
}
...
//  console.log(re)
  switch (re[0]) {
      ...
      case "re.loop":
          let r1 = mznRegExp(re[1]), l = re[2], u = re[3];
          let r2 = "(" + r1.repeat(l) + ")";
          if (u === undefined)
              r2 += "(" + r1 + ")*";
          else
