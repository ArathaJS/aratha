...
function preProcess(expr, model) {
    ...
    else if (expr instanceof NumberIsFinite || expr instanceof IsNaN)
        preProcess(expr.val, model);
...
function mznLiteral(x) {
    ...
    for (let i = 0; i < x.length; ++i) {
        ...
        if (ch < 32) {
            ...
            console.log("Warning! UTF-16 not supported yet by G-Strings");
            ascii = false;
            s += ("\\\\u" + ch.toString(16));
        }
    }
...
function mznRegExp(re) {
  ...
  switch (re[0]) {
      ...
      case "re.loop":
          ...
          //FIXME: Overapproximation, it includes line terminators!
          if (String(re[1]) === DOT) {
              const a = "\\\\x00", b = "\\\\xFF"
              if (typeof(u) === "number")
                  return "[" + a + "-" + b + "," + l + "," + u + "]";
