...
//    console.log(smt_cons)
    ...
    if (cons instanceof Array) {
        switch (cons[0]) {
            case 'is-Str':
                model.body += smt_cons + "TYPE_" + cons[1] + " = Str;\n";
                return;
            case 'is-Num':
                model.body += smt_cons + "TYPE_" + cons[1] + " = Num;\n";
                return;
            case 'is-Boolean':
                model.body += smt_cons + "TYPE_" + cons[1] + " = Bool;\n";
                return;
            default:
...
function preProcess(expr, model) {
    if (expr instanceof NewArray) {
        ...
        model.N_EXPL_PROPS++;
        model.N_WRITES++;
        model.body += "constraint put_prop(" + toTypeLoc(expr.base) + ", " + 
            toSval(expr.offset) + ", " + toTypeRepLoc(expr.val) + 
        ...
        model.N_EXPL_PROPS++;
        model.N_WRITES++;
        model.body += "constraint del_prop(" + toTypeLoc(expr.base) + ", " + 
...
//                case "function":
                default:
                    ...
                    return "P_" + suff + "S[get_prop(Object, " + 
            ...
            return "P_" + suff + "S[get_prop(" + toTypeLoc(expr.base) + ", "
...
function toTypeRep(x) {
    return toType(x)  + ", "  + toSval(x)
}
function toTypeLoc(x) {
    return toType(x)  + ", "  + toAddr(x)
}
function toTypeReps(x, y) {
    return toTypeRep(x) + ", "  + toTypeRep(y)
}
function toTypeRepLoc(x) {
    return toType(x)  + ", "  + toSval(x)  + ", " + toAddr(x);
}
function toTypeRepLocs(x, y) {
    return toTypeRepLoc(x)  + ", "  + toTypeRepLoc(y);
}
...
function arithBop(op, x, y) {
    switch (op) {
        case "+":
            return "plus_REP(" + toTypeReps(x, y) + ")";
        case "-":
...
function notBinaryRelToMZN(op, left, right) {
    switch (op) {
        case "<":
            return "not_lt(" + toTypeReps(left, right) + ")";
        case "<=":
            return "not_le(" + toTypeReps(left, right) + ")";
        case ">":
            return "not_lt(" + toTypeReps(right, left) + ")";
        case ">=":
            return "not_le(" + toTypeReps(right, left) + ")";
        default:
...
function binaryRelToMZN(op, left, right) {
    switch (op) {
        case "<":
            return "lt(" + toTypeReps(left, right) + ")";
        case "<=":
            return "le(" + toTypeReps(left, right) + ")";
        case ">":
            return "lt(" + toTypeReps(right, left) + ")";
        case ">=":
            return "le(" + toTypeReps(right, left) + ")";
        case "==":
            return "abstract_eq(" + toTypeRepLocs(left, right) + ")";
        case "===":
            return "strict_eq(" + toTypeRepLocs(left, right) + ")";
        case "!=":
            return "not(abstract_eq(" + toTypeRepLocs(left, right) + "))";
        case "!==":
            return "not(strict_eq(" + toTypeRepLocs(left, right) + "))";
        case "in":
            return "is_def("+ toSval(left) + ", " + toTypeLoc(right) +")";
        default:
