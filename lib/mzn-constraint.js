const {
    ...
    NumberIsNaN, IsFinite, IsNaN, StringToString, StringToLowerCase
} = require("./symbolic");
...
function preProcess(expr, model) {
    if (expr instanceof NewArray) {
        ...
        if (expr.base instanceof Constant) {
            ...
            if (!["Null", "Undefined", "Bool", "Number", "Str"].includes(t)) {
                model.N_IMPL_PROPS++;
                const i = model.N_IMPL_PROPS;
                model.body +=
                    "constraint if is_obj(" + t + ") then " + 
...
//                case "function":
    ...
    else if (expr instanceof StringSlice) {console.log(expr)
        if (suff === "TYPE")
            return "Str"
        else if (suff === "SVAL") {
            const s = toSval(expr.base);
            const i0 = toNum(expr.start);
            const j0 = isNaN(Number(i0))
                ? "if (is_neg(" + i0 + ")) then -str2nat(negate(" + i0 + 
                ")) + str_len(" + s + ") else str2nat(" + i0 + ") endif"
                : i0.slice(1, i0.length - 1);
            const i1 = toNum(expr.end);
            const j1 = isNaN(Number(i1))
                ? "if " + toType(expr.end) + " = Undef then "
                    + "if (is_neg(" + i1 + ")) then -str2nat(negate(" + i1 + "))" 
                    + " + str_len(" + s + ") else str_len(" + s + ") endif " +
                "else if (is_neg(" + i1 + ")) then -str2nat(negate(" + i1 + 
                ")) + str_len(" + s + ") else str2nat(" + i1 + ") endif endif"
                : i1.slice(1, i1.length - 1);
            return "str_sub(" + s + ", " + j0 + " + 1, " + j1 + ")";
        }
        else
            return TMP_ADDR[expr] === undefined ? "0" : TMP_ADDR[expr];
    }
       ...
       if (suff === "SVAL")
           // TODO: 
           return "split_SVAL(" + toSval(expr.base) + ", " + toSval(expr.sep) 
               + ", " + l + ")";
       else
           // TODO:
        ...
        if (suff === "TYPE")
            ...
            return toSval(expr.base); //FIXME: TODO.
        else
            return TMP_ADDR[expr] === undefined ? "0" : TMP_ADDR[expr];
    }
    else {
