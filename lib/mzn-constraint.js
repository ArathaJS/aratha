...
function toRep(expr, solver) {
    if (expr instanceof Constant)
        ...
        return binaryOpToMZN(expr.op, expr.left, expr.right, "REP", solver);
             ...
             expr.startsWith("add_")   || expr.startsWith("sub_") || 
             expr.startsWith("mul_")   || expr.startsWith("div_"))
        return expr;  
    else
...
function arithBop(op, x, y, s) {
    switch (op) {
        ...
        case "-":
            return "add_REP(" + toNum(x, s) + ", " + negate(toNum(y, s)) + ")"; //FIXME: sub_REP
        case "*":
            return "mul_REP(" + toNum(x, s) + ", " + toNum(y, s) + ")";
        case "/":
            return "div_REP(" + toNum(x, s) + ", " + toNum(y, s) + ")";
    }
}
