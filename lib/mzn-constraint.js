...
//                case "function":
            ...
            if (expr.offset.value === "length") {
                if (suff === "TYPE")
                    return "if " + toType(expr.base) + ' = Array then Num else ' + val + ' endif';
                else if (suff === "SVAL")
                    return "if " + toType(expr.base) + ' = Array then nat2str(array_length(' + 
                        toAddr(expr.offset) + ', N_IMPL_PROPS + ' + N_WRITES + ')) else ' + val + ' endif';
                else
                    return "if " + toType(expr.base) + ' = Array then 0 else ' + val + ' endif';
            }
            else
                // FIXME: If we have x[y] with x, y symbolic variables, we do not set the length 
                // builtin for Array objects.
                return val;
        }
    }
    else if (expr instanceof PutField)
