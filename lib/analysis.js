...
(function(sandbox) {
    ...
    const { Constant, Variable, Binary, Unary, GetField, PutField, DeleteField, ToString, StringToString, StringConcat, StringCharAt, StringSubstring, StringSlice, StringIndexOf, StringSearch, StringReplace, RegExpTest, ObjectConstruct, ObjectCast, ArrayConstruct, StringObjectConstruct, StringCast } = require("./symbolic");
    ...
    class Jalangi2DSEAnalysis {
        ...
        invokeFunPre(iid, f, base, args, isConstructor) {
            if (isConcolic(f)) {
                ...
                if (symF instanceof GetField) {
                    const keyVal = String(symF.offset.eval(this.inputs));
                    if (symF.base instanceof Constant) {
                        for(const k in Object.keys(symF.base.value)) {
                            if (k !== keyVal) {
                                this.path.addConstraint(new Binary("===", new ToString(symF.offset), new Constant(k)), false);
                            }
                        }
                    }
                    this.path.addConstraint(new Binary("===", new ToString(symF.offset), new Constant(keyVal)), true);
                }
            }

            const concF = getConcrete(f);
            if (concF === undefined || concF === null || isFunctionInstrumented(concF)) {
                return {f: concF, base: base, args: args};
            }

            const override = getOverride(concF, base, {inputs: this.inputs,
