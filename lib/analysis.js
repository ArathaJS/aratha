        ...
        async runAnalysis(maxIterations, cb) {
            ...
            try {
                ...
                try {
                    ...
                    const searcher = new DSE(solver, async (newInput) => {
                        while (true) {      
                            if (!first) {
                                fs.writeSync(inputLog, ",\n");
                            }
                            ...
                            try {
                                ...
                                // FIXME: Really ugly workaround for type errors. Better to treat method invocations 
                                //        as type constraints?
                                if (solver.isCPSolver() && e instanceof TypeError) {                                    
                                    const { JSTypes, parseSolution } = require("./cp");
                                    let sat = false;
                                    for (let [key, val] of Object.entries(solver.solution)) {
                                        if (/var\d/.test(key)) {
                                            const t = JSTypes[val.TYPE - 1];
                                            console.log("Type Error! Trying with type != " + t + " for variable " + key)
                                            solver.info = undefined;
                                            solver.solution = {};
                                            const status = await solver.runSolver("constraint TYPE_" + key + " != " + t + ";\n");
                                            if (status === "sat") {
                                                try {
                                                    var model = parseSolution(solver.getSolution());
                                                }
                                                catch (e) {
                                                    continue;
                                                }
