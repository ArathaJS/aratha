...
(function(sandbox) {
    ...
    const { Variable, Binary, Unary, GetField, PutField, DeleteField, StringToString, StringConcat, StringCharAt, StringSubstring, StringSlice, StringIndexOf, StringSearch, StringReplace, RegExpTest } = require("./symbolic");
    ...
    class Jalangi2DSEAnalysis {
        async runAnalysis(maxIterations, cb) {
            ...
            const inputLog = fs.openSync("inputlog.txt", "w");
            try {
                const searcher = new DSE(solver, (newInput) => {
                    ...
                    if (!searcher._visitedPaths.hasPrefix(this.path.constraints)) {
