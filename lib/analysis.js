...
(function(sandbox) {
    ...
    const CP_OPTS = {
        ...
        'twoSteps': true,
        'solve': 'length', //'num_len', 'typs', 'typ_len', 'str_def', 'reps'
        'debug': true,
        'abort': false, //true,
        'verifyModel': true //false,
    }
...
//            console.log(new Type(type), concVal)
    ...
    function createSolver(commandLog = null) {
        ...
        if (solver.isCPSolver()) {
            ...
            const theoryFiles = [SOLVER + "/prelude.smt2", "common.smt2", SOLVER + "/bitwise.smt2"];
    ...
    class Jalangi2DSEAnalysis {
        async runAnalysis(maxIterations, cb) {
            ...
            sandbox.choose = (funcs) => {
                funcs = getConcrete(funcs);
                if (funcs.length > 1) {
                    let i;
