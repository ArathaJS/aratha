...
(function(sandbox) {
    ...
    const { Variable, Binary, Unary, GetField, PutField, DeleteField, StringToString, StringConcat, StringCharAt, StringIndexOf } = require("./symbolic");
    ...
    overrides.set(String.prototype.toString, function() {
        const cbase = getConcrete(this), sbase = getSymbolic(this);
        return new Concolic(String.prototype.toString.call(cbase), new StringToString(sbase));
    });

    overrides.set(String.prototype.valueOf, function() {
        const cbase = getConcrete(this), sbase = getSymbolic(this);
        return new Concolic(String.prototype.valueOf.call(cbase), new StringToString(sbase));
    });
    ...
    overrides.set(String.prototype.charAt, function(params, idx) {
        const cbase = getConcrete(this), sbase = getSymbolic(this);
        const cidx = getConcrete(idx), sidx = getSymbolic(idx);
        // TODO: add type constraint on the base as per spec.
        return new Concolic(String.prototype.charAt.call(cbase, cidx), new StringCharAt(sbase, sidx));
    });

    overrides.set(String.prototype.indexOf, function(params, searchString, offset) {
        const cbase = getConcrete(this), sbase = getSymbolic(this);
        const csearchString = getConcrete(searchString), ssearchString = getSymbolic(searchString);
        const coffset = getConcrete(offset), soffset = getSymbolic(offset);
        return new Concolic(String.prototype.indexOf.call(cbase, csearchString, coffset), new StringIndexOf(sbase, ssearchString, soffset));
    });
