...
function isConcolic(val) {
    return val instanceof Concolic || (_.isObject(val) && val.hasOwnProperty(SYMBOLIC));
}
...
exports.clearSymbolic = function(val) {
    if(val instanceof Concolic)

    if (_.isObject(val) && SYMBOLIC in val) {
        delete val[SYMBOLIC];
    }
}
